...
def wb(obs, config, c, r) or 0:
    """Get wall bitfield from memory. Returns 0 if unknown (passable)."""
    return STATE["walls"].get((c, r), 0)
...
def factory_action(uid, data, obs, config, actions, reserved, occupied, my_player):
    ...
    # ── JUMP ── priority when in danger or stuck
    if jump_cd == 0 and turn > 2 and in_bounds(c, r + 2, obs, config):
        ...
        if should_jump:
            lr = r + 2
            landing = wb(obs, config, c, lr) if wb(obs, config, c, lr) is not None else 0
            if landing == 0 or any(can_go(c, lr, d, obs, config) for d in ("NORTH", "EAST", "WEST")):
    ...
    if move_cd == 0:
        ...
        # BFS to row ahead
        target_row = min(obs.northBound, r + 2)
        step = bfs_to_row((c, r), target_row, obs, config)
        if step:
            dc, dr, _ = DIRS[step]
            if dr >= 0:
    ...
    # ── BUILD ── skip during emergency
    if move_cd != 0 and build_cd == 0 and gap >= 2 and not emergency:
        ...
        if spawn_ok:
            ...
            if not friendly_at(occupied, spawn, my_player):
                ...
                # Miner when mine found (highest ROI)
                if has_nodes and miner_count < 2 and energy >= miner_cost + 100:
                    actions[uid] = "BUILD_MINER"
                    reserved.add(spawn)
                    return

                # Scout for exploration + crystal income
                if scout_count < 2 and energy >= scout_cost + 800:
                    actions[uid] = "BUILD_SCOUT"
                    reserved.add(spawn)
                    return

                # Worker for wall clearing
                if worker_count < 1 and energy >= worker_cost + 100:
