...
def factory_action(uid, data, obs, config, actions, reserved, occupied, my_player):
    ...
    if not skip_jump and jump_cd == 0 and in_bounds(c, r + 2, obs, config):
        vis_nodes = set(parse_key(k) for k in (getattr(obs, "miningNodes", {}) or {}))
        ex_mines = set(parse_key(k) for k in getattr(obs, "mines", {}).keys())
        if (c, r + 2) in vis_nodes and (c, r + 2) not in ex_mines:
            if can_go(obs, config, c, r, "NORTH") and can_go(obs, config, c, r + 1, "NORTH"):
                skip_jump = True
    ...
    if not skip_tier12 and not skip_nav_for_mine:
        ...
        if move_cd == 0:
            step_dir, _ = bfs_first_step((c, r), goals, obs, config, can_go, max_nodes=500)
            if step_dir:
                dc2, dr2, _ = DIRS[step_dir]
                if dr2 >= 0:
                    if factory_try_move(uid, c, r, step_dir, obs, config, actions, reserved, occupied, my_player,
