"""Train factory policy with Behavioral Cloning pre-training + PPO.

Phase 1: BC — collect expert data from agent_v2, supervised pre-training.
Phase 2: PPO — fine-tune with clipped surrogate objective + value baseline.
"""
...
def run_ppo_game(model, seed, explore=True):
    """Run one game, collecting (features, action, mask, log_prob, value, step_info)."""
    STATE_V1.update({"turn": 0, "nodes": set(), "last_factory_pos": None, "factory_stuck": 0})
    env = make("crawl", configuration={"randomSeed": seed}, debug=True)
    traj = []
...
def train(num_iter=200, batch=50, lr=0.0003, version=None,
    ...
    """Full training: BC pre-training + PPO fine-tuning."""
    if version is None:
        version = _next_version()
    save_path = f"nn_weights_v{version}.pt"
    print(f"=== Train v{version} (BC+PPO) ===")
