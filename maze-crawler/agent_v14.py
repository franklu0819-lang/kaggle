...
def factory_action(uid, data, obs, config, actions, reserved, occupied, my_player):
    ...
    # Skip JUMP if landing on a mining node and factory north is clear (let urgent mine handle it)
    if not skip_jump and jump_cd == 0 and in_bounds(c, r + 2, obs, config):
        vis_nodes = set(parse_key(k) for k in (getattr(obs, "miningNodes", {}) or {}))
        ex_mines = set(parse_key(k) for k in getattr(obs, "mines", {}).keys())
        landing_mine = (c, r + 2) in vis_nodes and (c, r + 2) not in ex_mines
        if landing_mine and can_go(obs, config, c, r, "NORTH"):
            skip_jump = True
